...
- Спроектировал доменную модель Avitosha v2, PostgreSQL-схему и транзакционный игровой репозиторий. Реализовал атомарный прогресс заданий, XP, уровней, предметов, истории, достижений и наград с идемпотентностью и защитой конкурентных сценариев.
- Разработал игровые API, дневные сводки, недельный лидерборд, характер питомца, безопасное переименование, reward balances и полный сценарий прохождения первой комнаты. Усилил покрытие тестами идемпотентности, конкурентности и сквозного игрового пути.
- Реализовал WebSocket-публикацию доменных событий и AI-советы по заданиям через ProxyAPI с безопасным fallback и ограничением передаваемого модели контекста.
- Провёл архитектурный аудит, разделил backend на gRPC-микросервисы, подготовил запускаемое окружение и оптимизировал сериализацию событий на основе CPU/memory-профилирования.
