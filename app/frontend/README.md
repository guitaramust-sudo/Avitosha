...
- [Структура](#структура)
- [Стили и адаптив](#стили-и-адаптив)
...
```text
Регистрация или вход → имя питомца и onboarding → игровой кабинет
→ выполнение задания → XP, награда и WebSocket-событие
→ AI-совет → новый предмет → размещение в комнате
```
...
- после первой регистрации пользователь задаёт имя питомцу и проходит интерактивное знакомство с заданиями, комнатой, коллекцией, лидербордом, валютой и ежедневными наградами.
...
- серию успешных дней, пять ежедневных заданий (2 покупателя, 2 продавца и 1 универсальное), цель «любые 2» и общий превью следующего дня;
- блок частых вопросов со свёрнутыми по умолчанию ответами.

Вспомогательные карточки можно сворачивать независимо друг от друга. Основная сцена, шапка и коллекция предметов всегда остаются доступны. Лидерборд показывает первые десять мест: на широком экране они распределены по двум колонкам, на мобильном — по одной.
...
Для выбранного задания frontend отдельно запрашивает:

```http
GET /api/v1/tasks/{task_id}/advice
```

`useTaskAdvice` хранит ответ в React Query пять минут. Совет отображается в окне деталей задания и в облачке над питомцем:

- сначала пользователь скрывает облачко текущей цели;
- через пять секунд появляется совет для задания;
- если цели при входе нет, совет показывается автоматически;
- после завершения всех квестов используется последнее выполненное задание;
- облачко совета можно скрыть тем же кликом по комнате;
- метка «ИИ» появляется только при `generatedByAi: true`.

Если AI-провайдер недоступен, backend возвращает безопасный локальный совет, поэтому интерфейс не зависит от доступности модели.
...
HomePage
  ...
  └── RoomWorkspace
        ├── dnd-kit → локальная раскладка комнаты
        ├── PixiJS → idle-анимация питомца
        └── useRoomSpeech → цель и отложенный совет
```
...
| Локальный state   | диалоги, свёрнутые панели, FAQ и последовательность облачков питомца        |
...
| `GET`   | `/api/v1/tasks/{task_id}/advice` | AI или fallback-совет Авитоши       |
...
│   ├── icons                  # логотипы и игровые иконки
│   ├── room-items             # предметы комнаты
│   └── screenshots            # изображения для документации
├── src
│   ├── api                    # HTTP-клиент и запросы
│   ├── components             # AuthForm, FAQ, карточки, диалоги и игровой UI
│   ├── hooks                  # auth, game, WebSocket, speech и drag-and-drop
...
До `1250px` боковая панель переносится под основной контент. На tablet карточки прогресса становятся одной вертикальной flex-колонкой. На мобильных шапка сохраняет игровые показатели, комната меняет пропорции, питомец и предметы удерживаются в безопасной области, коллекция перестраивается в сетку, лидерборд становится одноколоночным, а диалог задания открывается как нижняя панель.
...
| `npm run profile:bundle` | сборка и отчёт по размеру frontend-ресурсов  |
...
- диалог задания;
- AI-советы и последовательность облачков цели/совета;
- FAQ и раскрытие ответов;
