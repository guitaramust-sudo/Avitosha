...
function AchievementsPanel({ view = 'all' }: AchievementsPanelProps) {
  ...
  const showAchievements = view !== 'tasks'

  return (
    <>
      <CollapsibleSection
        as="aside"
        className={`achievements-panel achievements-panel--${view}`}
        title={
          view === 'tasks'
            ? 'Задания'
            : view === 'achievements'
        ...
        tourId={showTasks ? 'tasks' : 'achievements'}
      >
        <section className="achievements-panel__main">
          ...
          {showAchievements && (
            <>
              <span className="panel-kicker">Коллекция успехов</span>
